namespace pygmalion::dynamics
{
	...
	class command_debugPromoCaptureTargets :
...
#include "../include_dynamics.h"	
	private:
		...
		void process(const playerType pl, const pieceType pc, const squareType sq) noexcept
		{
			if constexpr (PLAYER < countPlayers)
			{
				...
				if (player == pl)
				{
					this->dumpSquares(generatorType::promoCaptureTargets(player, pc, sq));
				}
				else
	...
	protected:
		...
		{
			if (cmd == "debug-promocapturetargets")
			{
		...
		{
			return "DEBUG-PROMOCAPTURETARGETS";
		}
	};
