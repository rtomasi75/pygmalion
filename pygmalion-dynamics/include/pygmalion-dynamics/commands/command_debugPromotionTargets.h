namespace pygmalion::dynamics
{
	...
	class command_debugPromotionTargets :
	...
	private:
		...
		void process(const playerType pl, const pieceType pc, const squareType sq) noexcept
		{
			if constexpr (PLAYER < countPlayers)
			{
				...
				if (player == pl)
				{
					this->dumpSquares(generatorType::promotionTargets(player, pc, sq));
				}
				else
