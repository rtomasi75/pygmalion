namespace pygmalion
{
	...
	private:
		...
		constexpr static void insertionSort(VALUE* pValues, SCORE* pScores, const size_t len)
		{
			if (len > 1)
			{
				Sort(pValues, pScores, len - 1);
				const SCORE x_s = pScores[len - 1];
				const VALUE x_m = pValues[len - 1];
				size_t j = len - 2;
				for (; (j >= 0) && (pScores[j] > x_s); j--)
				{
					pScores[j + 1] = pScores[j];
