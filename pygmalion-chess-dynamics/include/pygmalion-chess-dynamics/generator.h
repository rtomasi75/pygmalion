	...
	public:
		...
		public:
			...
			PYGMALION_INLINE stack(boardType& position, historyType& history, contextType* pContext, const materialTableType& materialTable, const deltaType& delta) noexcept :
				pygmalion::generator<descriptor_dynamics, generator>::stack<PLAYER>(position, history, pContext, materialTable, delta)
			{
				generatorType::pawnTable().template prefetch<PLAYER>(*this);
			}
...
#include <pygmalion-dynamics/include_dynamics.h>
		...
		public:
			...
			public:
				...
				template<size_t PLAYER>
				PYGMALION_INLINE const typename generatorType::tropismType& kingTropism() const noexcept
				{
					constexpr const playerType pl{ static_cast<playerType>(PLAYER) };
					constexpr const playerType npl{ pl.next() };
					if (!(m_Flags & flagsKingTropism[pl]))
					{
						m_KingTropism[pl].template compute<static_cast<size_t>(npl)>(m_KingSquare[pl], m_Pawns[npl], m_Pawns[pl], m_KingSquare[npl], m_KingSquare[pl]);
						m_Flags |= flagsKingTropism[pl];
					}
					return m_KingTropism[pl];
				}
				template<size_t PLAYER>
				PYGMALION_INLINE const typename generatorType::tropismType& kingAreaTropism() const noexcept
				{
					constexpr const playerType pl{ static_cast<playerType>(PLAYER) };
					constexpr const playerType npl{ pl.next() };
					if (!(m_Flags & flagsKingTropism[pl]))
					{
						m_KingAreaTropism[pl].template compute<static_cast<size_t>(npl)>(generatorType::kingArea(m_KingSquare[pl]), m_Pawns[npl], m_Pawns[pl], m_KingSquare[npl], m_KingSquare[pl]);
	...
	private:
		...
		constexpr static inline const movebitsType m_NullMove{ motorType::move().createNull() };
	...
	private:
		...
		static void generateCriticalKnightMoves(const stackType<PLAYER>& stack, movelistType& moves) noexcept
		{
			...
			const squaresType criticalCaptures{ criticalSquares & stack.position().playerOccupancy(stackType<PLAYER>::NextPlayer) };
			...
			for (const auto from : discoveryCandidates)
			{
				...
				if (discoveredAttacks)
				{
					...
					case king:
					{
						...
						const squaresType discoveryCaptures{ discoveryTargets & stack.position().playerOccupancy(nextPlayer) };
					...
					case pawn:
					{
						if constexpr (movingPlayer == whitePlayer)
						{
							...
							const squaresType discoveryCaptures{ movegenPawnCaptureWhite.targets(from,all) & allowedDestinations2 & stack.position().playerOccupancy(nextPlayer) };
						...
						else
						{
							...
							const squaresType discoveryCaptures{ movegenPawnCaptureBlack.targets(from,all) & allowedDestinations2 & stack.position().playerOccupancy(nextPlayer) };
		...
		template<size_t PLAYER>
		static squaresType tropismPawn(const squareType sq, const squaresType& unoccupiedSquares, const squaresType& victims) noexcept
		{
			constexpr const playerType pl{ static_cast<playerType>(PLAYER) };
			constexpr const squaresType all{ squaresType::all() };
			squaresType tropism;
			const rankType rank{ sq.rank() };
			if constexpr (pl == whitePlayer)
			{
				if (victims[sq])
		...
		template<size_t PLAYER>
		static squaresType tropismPawn(const squaresType& sq, const squaresType& unoccupiedSquares, const squaresType& victims) noexcept
		{
			constexpr const playerType pl{ static_cast<playerType>(PLAYER) };
			constexpr const squaresType all{ squaresType::all() };
			constexpr const squaresType none{ squaresType::none() };
	...
	public:
		...
		static void attackTropismKing(const squareType sq, const squaresType& allowedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
		...
		template<unsigned MAXDIST>
		static void attackTropismKing(const squaresType& sq, const squaresType& allowedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
			tropism[0] = tropismKing(sq);
			if constexpr (MAXDIST > 0)
			{
		...
		template<unsigned MAXDIST>
		static void attackTropismKnight(const squareType sq, const squaresType& allowedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
		...
		template<unsigned MAXDIST>
		static void attackTropismKnight(const squaresType& sq, const squaresType& allowedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
			tropism[0] = tropismKnight(sq);
			if constexpr (MAXDIST > 0)
			{
		...
		template<unsigned MAXDIST>
		static void attackTropismRook(const squareType sq, const squaresType& allowedSquares, const squaresType& unoccupiedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
		...
		template<unsigned MAXDIST>
		static void attackTropismRook(const squaresType& sq, const squaresType& allowedSquares, const squaresType& unoccupiedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
			tropism[0] = tropismRook(sq, unoccupiedSquares);
			if constexpr (MAXDIST > 0)
			{
		...
		template<unsigned MAXDIST>
		static void attackTropismBishop(const squareType sq, const squaresType& allowedSquares, const squaresType& unoccupiedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
		...
		template<unsigned MAXDIST>
		static void attackTropismBishop(const squaresType& sq, const squaresType& allowedSquares, const squaresType& unoccupiedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
			tropism[0] = tropismBishop(sq, unoccupiedSquares);
			if constexpr (MAXDIST > 0)
			{
		...
		template<unsigned MAXDIST>
		static void attackTropismQueen(const squareType sq, const squaresType& allowedSquares, const squaresType& unoccupiedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
		...
		template<unsigned MAXDIST>
		static void attackTropismQueen(const squaresType& sq, const squaresType& allowedSquares, const squaresType& unoccupiedSquares, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
			tropism[0] = tropismQueen(sq, unoccupiedSquares);
			if constexpr (MAXDIST > 0)
			{
		...
		template<unsigned MAXDIST, size_t PLAYER>
		static void attackTropismPawn(const squareType sq, const playerType pl, const squaresType& allowedSquares, const squaresType& unoccupiedSquares, const squaresType& victims, const std::array<squaresType, MAXDIST + 1>& tropismKnight, const std::array<squaresType, MAXDIST + 1>& tropismBishop, const std::array<squaresType, MAXDIST + 1>& tropismRook, const std::array<squaresType, MAXDIST + 1>& tropismQueen, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
			...
			constexpr const rankType promotionRank{ (player == whitePlayer) ? rank8 : rank1 };
			if constexpr (MAXDIST > 0)
			{
				std::array<squaresType, MAXDIST> visited;
				visited[0] = tropism[0];
				for (unsigned i = 1; i < MAXDIST; i++)
				{
					const squaresType tropismPromotion{ (tropismKnight[i - 1] | tropismBishop[i - 1] | tropismRook[i - 1] | tropismQueen[i - 1]) & promotionRank };
					tropism[i] = tropismPawn((tropism[i - 1] | tropismPromotion) & allowedSquares, player, unoccupiedSquares, victims) & ~visited[i - 1];
					visited[i] = visited[i - 1] | tropism[i];
				}
				const squaresType tropismPromotion2{ (tropismKnight[MAXDIST - 1] | tropismBishop[MAXDIST - 1] | tropismRook[MAXDIST - 1] | tropismQueen[MAXDIST - 1]) & promotionRank };
				tropism[MAXDIST] = tropismPawn((tropism[MAXDIST - 1] | tropismPromotion2) & allowedSquares, player, unoccupiedSquares, victims) & ~visited[MAXDIST - 1];
			}
		}
		template<unsigned MAXDIST, size_t PLAYER>
		static void attackTropismPawn(const squaresType& sq, const squaresType& allowedSquares, const squaresType& unoccupiedSquares, const squaresType& victims, const std::array<squaresType, MAXDIST + 1>& tropismKnight, const std::array<squaresType, MAXDIST + 1>& tropismBishop, const std::array<squaresType, MAXDIST + 1>& tropismRook, const std::array<squaresType, MAXDIST + 1>& tropismQueen, std::array<squaresType, MAXDIST + 1>& tropism) noexcept
		{
			constexpr const playerType player{ static_cast<playerType>(PLAYER) };
			constexpr const squaresType all{ squaresType::all() };
			if constexpr (player == whitePlayer)
			...
			if constexpr (MAXDIST > 0)
			{
				...
				{
					const squaresType tropismPromotion{ (tropismKnight[i - 1] | tropismBishop[i - 1] | tropismRook[i - 1] | tropismQueen[i - 1]) & promotionRank };
					tropism[i] = tropismPawn<PLAYER>((tropism[i - 1] | tropismPromotion) & allowedSquares, unoccupiedSquares, victims) & ~visited[i - 1];
					visited[i] = visited[i - 1] | tropism[i];
				}
				const squaresType tropismPromotion2{ (tropismKnight[MAXDIST - 1] | tropismBishop[MAXDIST - 1] | tropismRook[MAXDIST - 1] | tropismQueen[MAXDIST - 1]) & promotionRank };
				tropism[MAXDIST] = tropismPawn<PLAYER>((tropism[MAXDIST - 1] | tropismPromotion2) & allowedSquares, unoccupiedSquares, victims) & ~visited[MAXDIST - 1];
			}
		}
		...
		static squaresType attackersBoth(const boardType& position, const squareType square) noexcept
		{
			PYGMALION_ASSERT(square.isValid());
			constexpr const squaresType none{ squaresType::none() };
		...
		template<size_t PLAYER>
		static void movesFromSquare(const stackType<PLAYER>& stack, const squareType square, squaresType& moves, squaresType& captures) noexcept
		{
			const boardType& position{ stack.position() };
			constexpr const squaresType none{ squaresType::none() };
			moves = none;
		...
		static bool parseSAN_Implementation(const std::string& san, const stackType<PLAYER>& stack, movebitsType& mv, size_t& count) noexcept
		{
			...
			count = 0;
			squareType to;
			squareType from{ squareType::invalid };
			squaresType origins;
			rankType originRank{ rankType::invalid };
			fileType originFile{ fileType::invalid };
			...
			if (!from.isValid())
			{
				...
				case king:
					if (bCapture)
						origins = generatorType::movegenKing.inverseAttacks(to, squaresType::all());
					else
						origins = generatorType::movegenKing.inverseTargets(to, squaresType::all());
					origins &= stack.position().pieceOccupancy(piece);
					origins &= stack.position().playerOccupancy(stackType<PLAYER>::MovingPlayer);
					break;
				case knight:
					if (bCapture)
						origins = generatorType::movegenKnight.inverseAttacks(to, squaresType::all());
					else
						origins = generatorType::movegenKnight.inverseTargets(to, squaresType::all());
					origins &= stack.position().pieceOccupancy(piece);
					origins &= stack.position().playerOccupancy(stackType<PLAYER>::MovingPlayer);
					break;
				case bishop:
					if (bCapture)
						origins = generatorType::movegenSlidersDiag.inverseAttacks(to, ~stack.position().totalOccupancy());
					else
						origins = generatorType::movegenSlidersDiag.inverseAttacks(to, ~stack.position().totalOccupancy());
					origins &= stack.position().pieceOccupancy(piece);
					origins &= stack.position().playerOccupancy(stackType<PLAYER>::MovingPlayer);
				...
				case rook:
					if (bCapture)
						origins = generatorType::movegenSlidersHV.inverseAttacks(to, ~stack.position().totalOccupancy());
					else
						origins = generatorType::movegenSlidersHV.inverseAttacks(to, ~stack.position().totalOccupancy());
					origins &= stack.position().pieceOccupancy(piece);
					origins &= stack.position().playerOccupancy(stackType<PLAYER>::MovingPlayer);
				...
				case queen:
					if (bCapture)
						origins = generatorType::movegenSlidersHV.inverseAttacks(to, ~stack.position().totalOccupancy()) | generatorType::movegenSlidersDiag.inverseAttacks(to, ~stack.position().totalOccupancy());
					else
						origins = generatorType::movegenSlidersHV.inverseAttacks(to, ~stack.position().totalOccupancy()) | generatorType::movegenSlidersDiag.inverseAttacks(to, ~stack.position().totalOccupancy());
					origins &= stack.position().pieceOccupancy(piece);
					origins &= stack.position().playerOccupancy(stackType<PLAYER>::MovingPlayer);
				...
				case pawn:
					if constexpr (stackType<PLAYER>::MovingPlayer == whitePlayer)
					{
						if (bCapture)
						{
							if (!stack.position().playerOccupancy(stackType<PLAYER>::NextPlayer)[to])
								origins = generatorType::movegenPawnCaptureWhite.inverseAttacks(to, squaresType::all());
							else
								origins = generatorType::movegenPawnCaptureWhite.inverseAttacks(to, squaresType::all()) | generatorType::movegenPawnPromoCaptureWhite.inverseAttacks(to, squaresType::all());
							origins &= stack.position().pieceOccupancy(piece);
							origins &= stack.position().playerOccupancy(stackType<PLAYER>::MovingPlayer);
						}
						else
					...
					else
					{
						...
						{
							if (!stack.position().playerOccupancy(stackType<PLAYER>::NextPlayer)[to])
								origins = generatorType::movegenPawnCaptureBlack.inverseAttacks(to, squaresType::all());
							else
								origins = generatorType::movegenPawnCaptureBlack.inverseAttacks(to, squaresType::all()) | generatorType::movegenPawnPromoCaptureBlack.inverseAttacks(to, squaresType::all());
							origins &= stack.position().pieceOccupancy(piece);
							origins &= stack.position().playerOccupancy(stackType<PLAYER>::MovingPlayer);
						}
						else
					...
					if constexpr (stackType<PLAYER>::MovingPlayer == whitePlayer)
					{
						...
						else
						{
							if (to.rank() != rank8)
							{
								if (bPromotion)
									return false;
								mv = motorType::move().createCapture(from, to);
							}
							else
							{
								if (!bPromotion)
								...
								default:
									return false;
								case knight:
									mv = motorType::move().createPromoCaptureKnight(from, to);
									break;
								case bishop:
									mv = motorType::move().createPromoCaptureBishop(from, to);
									break;
								case rook:
									mv = motorType::move().createPromoCaptureRook(from, to);
									break;
								case queen:
									mv = motorType::move().createPromoCaptureQueen(from, to);
									break;
								}
							}
						}
		...
		template<size_t PLAYER>
		static std::string moveToSAN_Implementation(const stackType<PLAYER>& stack, const movebitsType mv) noexcept
		{
			const boardType& position{ stack.position() };
			const squareType from{ motorType::move().fromSquare(position,mv) };
			const squareType to{ motorType::move().toSquare(position,mv) };
		...
		constexpr static size_t movegenStage_Implementation(const movegenPhase phase, const size_t stageIndex) noexcept
		{
			...
			case movegenPhase::QSPhase1:
				...
				{
				case 0:
					return movegenStage_QuietCriticalMoves;
				case 1:
					return movegenStage_WinningMoves;
				case 2:
					return movegenStage_EqualMoves;
		...
		static inline std::array<std::array<std::array<squaresType, countSquares>, countPieces>, countPlayers> m_PromotionTargets
		{
			arrayhelper::generate<countPlayers,std::array<std::array<squaresType, countSquares>,countPieces>>([](const size_t PLAYER)
			{
				return arrayhelper::generate<countPieces,std::array<squaresType, countSquares>>([PLAYER](const size_t PIECE)
					{
						return arrayhelper::generate<countSquares, squaresType>([PLAYER, PIECE](const size_t SQUARE)
							{
								const playerType pl{ static_cast<playerType>(PLAYER) };
								const squareType from{ static_cast<playerType>(SQUARE) };
								const pieceType pc{ static_cast<pieceType>(PIECE) };
								constexpr const squaresType none{ squaresType::none() };
								switch (pc)
								{
								default:
								...
								case pawn:
									if (pl == whitePlayer)
										return movegenPawnPromotionWhite.untabled_targets(from, squaresType::all());
									else
										return movegenPawnPromotionBlack.untabled_targets(from, squaresType::all());
								}
							});
					});
			})
		};
		static inline std::array<std::array<std::array<squaresType, countSquares>, countPieces>, countPlayers> m_PromoCaptureTargets
		{
			arrayhelper::generate<countPlayers,std::array<std::array<squaresType, countSquares>,countPieces>>([](const size_t PLAYER)
			{
				return arrayhelper::generate<countPieces,std::array<squaresType, countSquares>>([PLAYER](const size_t PIECE)
						...
						return arrayhelper::generate<countSquares, squaresType>([PLAYER, PIECE](const size_t SQUARE)
							{
								...
								case pawn:
									if (pl == whitePlayer)
										return movegenPawnPromoCaptureWhite.untabled_targets(from, squaresType::all());
									else
										return movegenPawnPromoCaptureBlack.untabled_targets(from, squaresType::all());
								}
							});
					});
			})
		};
		static inline std::array<std::array<std::array<squaresType, countSquares>, countPieces>, countPlayers> m_QuietTargets
		{
			arrayhelper::generate<countPlayers,std::array<std::array<squaresType, countSquares>,countPieces>>([](const size_t PLAYER)
			{
				return arrayhelper::generate<countPieces,std::array<squaresType, countSquares>>([PLAYER](const size_t PIECE)
						...
						return arrayhelper::generate<countSquares, squaresType>([PLAYER, PIECE](const size_t SQUARE)
							{
								...
								default:
									return none;
								case knight:
									return movegenKnight.untabled_targets(from, squaresType::all());
								case king:
									return movegenKing.untabled_targets(from, squaresType::all());
								case rook:
									return movegenSlidersHV.untabled_targets(from, squaresType::all());
								case bishop:
									return movegenSlidersDiag.untabled_targets(from, squaresType::all());
								case queen:
									return movegenSlidersHV.untabled_targets(from, squaresType::all()) | movegenSlidersDiag.untabled_targets(from, squaresType::all());
								case pawn:
									if (pl == whitePlayer)
										return movegenPawnPushWhite.untabled_targets(from, squaresType::all()) | movegenPawnDoublePushWhite.untabled_targets(from, squaresType::all());
									else
										return movegenPawnPushBlack.untabled_targets(from, squaresType::all()) | movegenPawnDoublePushBlack.untabled_targets(from, squaresType::all());
								}
							});
					});
			})
		};
		static inline std::array<std::array<std::array<squaresType, countSquares>, countPieces>, countPlayers> m_CaptureTargets
		{
			arrayhelper::generate<countPlayers,std::array<std::array<squaresType, countSquares>,countPieces>>([](const size_t PLAYER)
			{
				return arrayhelper::generate<countPieces,std::array<squaresType, countSquares>>([PLAYER](const size_t PIECE)
						...
						return arrayhelper::generate<countSquares, squaresType>([PLAYER, PIECE](const size_t SQUARE)
							{
								...
								default:
									...
									return none;
								case knight:
									return movegenKnight.untabled_attacks(from, squaresType::all());
								case king:
									return movegenKing.untabled_attacks(from, squaresType::all());
								case rook:
									return movegenSlidersHV.untabled_attacks(from, squaresType::all());
								case bishop:
									return movegenSlidersDiag.untabled_attacks(from, squaresType::all());
								case queen:
									return movegenSlidersHV.untabled_attacks(from, squaresType::all()) | movegenSlidersDiag.untabled_attacks(from, squaresType::all());
								case pawn:
									if (pl == whitePlayer)
										return movegenPawnCaptureWhite.untabled_attacks(from, squaresType::all());
									else
										return movegenPawnCaptureBlack.untabled_attacks(from, squaresType::all());
								}
							});
					});
			})
		};
		static inline std::array<std::array<squaresType, countPieces>, countPlayers> m_PromotionOrigins
		{
			arrayhelper::generate<countPlayers,std::array<squaresType,countPieces>>([](const size_t PLAYER)
				{
					return arrayhelper::generate<countPieces,squaresType>([PLAYER](const size_t PIECE)
						{
							const playerType pl{ static_cast<playerType>(PLAYER) };
							const pieceType pc{ static_cast<pieceType>(PIECE) };
							constexpr const squaresType none{ squaresType::none() };
							...
							case pawn:
								if (pl == whitePlayer)
									return movegenPawnPromotionWhite.allowedFromSquares();
								else
									return movegenPawnPromotionBlack.allowedFromSquares();
							}
						});
				})
		};
		static inline std::array<std::array<squaresType, countPieces>, countPlayers> m_PromoCaptureOrigins
		{
			arrayhelper::generate<countPlayers,std::array<squaresType,countPieces>>([](const size_t PLAYER)
				{
					return arrayhelper::generate<countPieces,squaresType>([PLAYER](const size_t PIECE)
						{
							...
							case pawn:
								if (pl == whitePlayer)
									return movegenPawnPromoCaptureWhite.allowedFromSquares();
								else
									return movegenPawnPromoCaptureBlack.allowedFromSquares();
							}
						});
				})
		};
		static inline std::array<std::array<squaresType, countPieces>, countPlayers> m_QuietOrigins
		{
			arrayhelper::generate<countPlayers,std::array<squaresType,countPieces>>([](const size_t PLAYER)
				{
					return arrayhelper::generate<countPieces,squaresType>([PLAYER](const size_t PIECE)
						{
							...
							default:
								return none;
							case knight:
								return movegenKnight.allowedFromSquares();
							case king:
								return movegenKing.allowedFromSquares();
							case rook:
								return movegenSlidersHV.allowedFromSquares();
							case bishop:
								return movegenSlidersDiag.allowedFromSquares();
							case queen:
								return movegenSlidersHV.allowedFromSquares() | movegenSlidersDiag.allowedFromSquares();
							case pawn:
								if (pl == whitePlayer)
									return movegenPawnPushWhite.allowedFromSquares() | movegenPawnDoublePushWhite.allowedFromSquares();
								else
									return movegenPawnPushBlack.allowedFromSquares() | movegenPawnDoublePushBlack.allowedFromSquares();
							}
						});
				})
		};
		static inline std::array<std::array<squaresType, countPieces>, countPlayers> m_CaptureOrigins
		{
			arrayhelper::generate<countPlayers,std::array<squaresType,countPieces>>([](const size_t PLAYER)
				{
					return arrayhelper::generate<countPieces,squaresType>([PLAYER](const size_t PIECE)
						{
							...
							case pawn:
								if (pl == whitePlayer)
									return movegenPawnCaptureWhite.allowedFromSquares();
								else
									return movegenPawnCaptureBlack.allowedFromSquares();
							}
						});
				})
		};
		...
		{
			return m_PromotionTargets[pl][pc][from];
		}
		...
		{
			return m_PromoCaptureTargets[pl][pc][from];
		}
		...
		{
			return m_QuietTargets[pl][pc][from];
		}
