	...
	private:
		...
		bool test(const testset& set, std::uintmax_t& nodes, typename profiler::durationType& duration) noexcept
		{
			if constexpr (PLAYER < countPlayers)
			{
				historyType history;
				boardType position;
				if (!position.setFen(set.fen(), this->stateEngine().materialTable()))
				{
				...
				if (player == position.movingPlayer())
				{
					...
					profiler p;
					p.start();
					stackType<PLAYER> stack{ stackType<PLAYER>(position, history, pContext, this->stateEngine().materialTable(), this->dynamicsEngine().delta()) };
